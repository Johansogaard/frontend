...
export const CartProvider = ({ children }: CartProviderProps) => {
  ...
  const removeItem = (itemId: number) => {
    setItems((prevItems) =>
      prevItems.filter((item) => item.product.product_id !== itemId),
    )
  }
  ...
  const calcTotalWithDiscount = () => {
    const totalMinusDiscount =
      parseFloat(calcTotal()) - parseFloat(calcTotalDiscount())
  ...
  const calcDiscountForItem = (item: Item) => {
    ...
    if (item.product.rebateQuantity && item.product.rebatePercent) {
      discount =
        (item.product.product_price * item.product.rebateQuantity) /
  ...
  const calcItemSubTotalWithDiscount = (item: Item): string => {
    const discountprice =
      item.quantity >= 5
        ? item.product.product_price * 0.8
