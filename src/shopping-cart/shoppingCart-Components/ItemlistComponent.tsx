...
export function ItemListComponent() {
  ...
  console.log("Cart Items:", state.items); // This will show the cart items in the console
...
function ShoppingCartItem({item}: {item:Item}) {
  ...
  const handleUpsell = (originalProductId: number, upsellProductId: number) => {
    if (upsellProductId != null || upsellProductId != undefined) {  // Check for null or undefined
      dispatch({ type: 'CART_UPSELL_PRODUCT', payload: { originalProductId, upsellProductId } });
    }};
  ...
  return (
    <section>
      ...
      <article className="item-container">
        <img src={item.product.product_image_url} alt="item image" className="item-image" />
        ...
        <div className="item-pricing">
          ...
          <div className='item-pricing-lineTwo'>
            <p className='price-pr-item-tag'>{item.product.product_price} per item</p>
            <div className="quantity-Checker">
              <button className='quantity-btn' onClick={() => handleProductQuantity(item.product.product_id, -1)}>-</button>
          ...
          {item.product.upsellProduct_id && (
            <button className="upsell-btn" onClick={() => item.product.upsellProduct_id ? handleUpsell(item.product.product_id, item.product.upsellProduct_id) : null}>
            Upgrade to Premium
          </button>
          )}
        </div>
